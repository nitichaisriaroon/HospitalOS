...
trigger_file="/opt/PostgreSQL/9.5/data/im_the_master"
standby_file="/opt/PostgreSQL/9.5/data/im_slave"
demote_host=""
...
while test $# -gt 0; do
    ...
    case "$1" in
        ...
        -h|--help)
            ...
            echo "    Optional, default: /opt/PostgreSQL/9.5/data/im_the_master"
            echo "-s, --standby_file=FILE   specify standby file path"
            echo "    Optional, default: /opt/PostgreSQL/9.5/data/im_slave"
...
if [ "$demote_host" != "" ]; then
    ...
    ssh -T postgres@$demote_host /opt/PostgreSQL/9.5/data/replscripts/disable_postgresql.sh -t $trigger_file -s $standby_file && success=true
...
if [ -e /opt/PostgreSQL/9.5/data/recovery.conf ]; then
     
    echo "INFO: Deleting recovery.conf file..."
 
    success=false
    rm /opt/PostgreSQL/9.5/data/recovery.conf && success=true
     
    if ! $success ; then
 
        echo "ERROR: Failed to delete '/opt/PostgreSQL/9.5/data/recovery.conf' file."
        exit 4
...
if [ -e /opt/PostgreSQL/9.5/data/postgresql.conf ]; then
 
    echo "INFO: postgresql.conf file found. Checking if it is for primary server..."
    if diff /opt/PostgreSQL/9.5/data/postgresql.conf /opt/PostgreSQL/9.5/data/repltemplates/postgresql.conf.primary >/dev/null ; then
        ...
        success=false
        rm /opt/PostgreSQL/9.5/data/postgresql.conf && success=true
             
        if ! $success ; then
 
            echo "ERROR: Failed to delete '/opt/PostgreSQL/9.5/data/postgresql.conf' file."
            exit 4
        ...
        success=false
        cp /opt/PostgreSQL/9.5/data/repltemplates/postgresql.conf.primary /opt/PostgreSQL/9.5/data/postgresql.conf && success=true
...
if [ -e /opt/PostgreSQL/9.5/data/primary_info ]; then
    rm /opt/PostgreSQL/9.5/data/primary_info
fi
 
echo "REPL_USER=${replication_user}\nREPL_PASSWORD=${replication_password}\nTRIGGER_FILE=${trigger_file}\nSTANDBY_FILE=${standby_file}\n" >> /opt/PostgreSQL/9.5/data/primary_info
 
chown postgres:postgres /opt/PostgreSQL/9.5/data/primary_info
chmod 0600 /opt/PostgreSQL/9.5/data/primary_info
